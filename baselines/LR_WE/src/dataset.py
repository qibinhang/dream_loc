...
def _trans_data_for_one_report(qid, bugid, all_src_info, top_n):
    ...
    for src_info in all_src_info:
        ...
        emb_sim_1.append(src_info['emb_sim_1'])
        emb_sim_2.append(src_info['emb_sim_2'])
    ...
    max_emb_1, min_emb_1 = max(emb_sim_1), min(emb_sim_1)
    max_emb_2, min_emb_2 = max(emb_sim_2), min(emb_sim_2)
    ...
    for src_info in final_src:
        one_data = f"{src_info['label']} qid:{qid} " \
                   ...
                   f"7:{cal_scaling(src_info['emb_sim_1'], max_emb_1, min_emb_1):.4f} " \
                   f"8:{cal_scaling(src_info['emb_sim_2'], max_emb_2, min_emb_2):.4f} " \
                   f"# {bugid}"
...
def generate_dataset(tag):
    ...
    for _, row in tqdm(report_corpus.iterrows(), desc=f'{tag} dataset', total=report_corpus.shape[0], ncols=100):
        ...
        for src_path in relative_src_path2commit_path:
            ...
            emb_sim_1, emb_sim_2 = get_embedding_sim(bugid, commit_path)
            ...
            data_for_each_report.append(
                [('src_path', src_path), ('label', label), ('sur_sim', sur_sim), ('api_sim', api_sim),
                 ...
                 ('emb_sim_1', emb_sim_1), ('emb_sim_2', emb_sim_2)]
            )
...
def get_embedding_sim(bugid, commit_path):
    value = embedding_sim[bugid][commit_path]
    return value
...
if __name__ == '__main__':
    ...
    embedding_sim = load_file(configs.embedding_sim_path)
